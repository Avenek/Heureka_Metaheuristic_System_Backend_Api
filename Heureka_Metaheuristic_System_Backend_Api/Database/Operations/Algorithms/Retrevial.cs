...
namespace Heureka_Metaheuristic_System_Backend_Api.Database.Operations.Algorithms
{
    public static partial class AlgorithmDatabaseOperations
    {
        ...
        public static async Task<Algorithm> GetAlgorithmWithParametersById(this DatabaseOperationExecutionService service, uint id)
        {
        ...
        public static async Task<List<Algorithm>> GetAlgorithmWithParametersByIds(this DatabaseOperationExecutionService service, IEnumerable<uint> ids)
        {
            var repositoryCollection = (RepositoryCollection)service.RepositoryCollection;
            var mapper = service.GetMappingService<DataDtoMappingService>().Mapper;
            var algorithms = await service.GetEntitiesBy<Algorithm>(a => ids.Contains(a.Id),
                    a => new Algorithm()
                    {
                        Id = a.Id,
                        FileName = a.FileName,
