...
namespace Heureka_Metaheuristic_System_Backend_Api.Reflection
{
    public class DllFileLoader
    {
        ...
        public object CreateInstance(string dllPath, string className)
        {
            ...
            if (type == null)
            {
                throw new BadRequestException($"Class '{className}' was not found in assembly '{assembly.FullName}'.");
            }
            ...
            if (instance == null)
            {
                throw new BadRequestException($"Could not create instance of class '{className}'.");
            }
        ...
        private Type FindMatchingType<T>(Assembly assembly)
        {
            ...
            var optimizationType = types.FirstOrDefault(
                type => type.GetInterfaces().Any(
                    interfaceType => ReflectionValidator.ImplementsInterface(
                        interfaceType,
                        typeof(T))));
